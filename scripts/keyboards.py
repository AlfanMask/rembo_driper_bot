...
def set_setting_mode_keyboard(active_mode: ai_assistant_mode) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{input_state.chatting[lang.id]} {'✅' if active_mode == ai_assistant_mode.chatting else ''}", callback_data=f"{input_state.chatting[lang.id]}-{input_state.flag_setting_mode_ai}")],
